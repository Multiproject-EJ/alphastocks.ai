...
- ☑ Populate `assets/data/` with 10 demo portfolios, watchlists, events, journal entries, analytics summaries matching schema.
  ...
  - 2025-11-01: Added `workspace/scripts/generateSupabaseSeed.js` to convert demo fixtures into SQL insert statements with optional CLI flags for alternate destinations or stdout. Mirrors Supabase's [seeding guide](https://supabase.com/docs/guides/database/seeding) and aligns with the CLI's [`supabase db seed`](https://supabase.com/docs/reference/cli/supabase-db-seed) workflow.
  - 2025-11-02: Replaced the monolithic `supabase/seed.sql` with patch-based schema files starting at `supabase/patches/001_core_schema.sql` to document only the structures currently needed by the workspace.
