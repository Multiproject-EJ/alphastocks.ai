...
- Updated AI Insights modal sort controls to show helper copy for the active sort option so ordering intent is explicit for players.
- Added focused config + modal helper tests covering sort-helper template guardrails and formatted helper-copy output.
...
**How to test:**
- `cd apps/investing-board-game-v3 && npm run test -- src/config/__tests__/aiInsights.test.ts src/components/__tests__/AIInsightsModal.test.ts`
...
- Added a shared threshold normalization helper that enforces finite, non-negative values and guarantees day-threshold ordering above the hour threshold.
- Updated the AI Insights modal to consume normalized threshold config and expanded config tests for threshold guardrail/fallback behavior.
...
- Updated AI insights config normalization + exported formatters to support day-based relative-age string assembly.
- Switched the AI Insights modal age formatter to render day-based copy for 24h+ insight timestamps and expanded config unit tests.
...
**Date:** 2026-02-12
**Slice:** P6.28 AI insights relative-age minutes template guardrails
**Summary:**
- Applied the shared `{minutes}` template guardrail to AI insights relative-age template normalization so override copy remains placeholder-safe.
- Updated config normalization to fall back to default relative-age copy whenever a custom minutes-ago template omits the `{minutes}` token.
- Added AI insights config unit coverage for valid and invalid relative-age minute templates.
...
- Updated AIInsightsModal to call the shared formatter for due-now cooldown copy instead of inline replacement chaining.
- Expanded AI insights config tests with coverage for default and reordered due-now template composition.
...
- Extended AI insights config typing/normalization and tests to enforce due-now separator token availability.
- Updated the AI Insights modal cooldown chip to compose due-now urgency emphasis + countdown copy using the configured separator token.
...
**How to test:**
- `cd apps/investing-board-game-v3 && npm run test -- src/config/__tests__/aiInsights.test.ts`
- (manual) Open Hub → Insights → AI Insights modal and verify due-now cooldown chips render urgency emphasis + countdown with the configured separator token while on-track chips remain unchanged.
...
**Date:** 2026-02-11
**Slice:** P6.38 AI insights relative-age day-count minimum token
**Summary:**
- Added a config-first `dayCountMinimum` token in AI insights relative-age settings so day labels keep a human-friendly floor when divisor tuning is aggressive.
- Extended AI insights config normalization with day-count minimum guardrails that keep values finite and at least one, with deterministic fallback behavior.
- Updated AI Insights day-label rendering to clamp computed day values against the new minimum token and expanded config tests for guardrails + surface coverage.
