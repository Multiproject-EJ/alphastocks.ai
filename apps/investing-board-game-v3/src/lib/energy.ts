 ...
 */
export function calculateRegeneratedRolls(lastCheckTime: Date, currentRolls: number = 0): number {
  const now = new Date()
 ...
 */
export function getResetRollsAmount(lastCheckTime: Date): number | null {
  const now = new Date()
  const minutesElapsed = (now.getTime() - lastCheckTime.getTime()) / (1000 * 60)
  const resetCycles = Math.floor(minutesElapsed / ENERGY_CONFIG.REGEN_INTERVAL_MINUTES)
  ...
  if (resetCycles >= 1) {
    return ENERGY_CONFIG.RESET_AMOUNT
  }
