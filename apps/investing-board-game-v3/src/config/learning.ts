import rawLearningConfig from '../../../../config/learning.json'
...
export interface LearningCategoryStyle {
  label: string
  textClass: string
  glowClass: string
  badgeClass: string
...
export interface LearningRewardConfig {
  baseStars: number
  baseXp: number
...
type LearningConfig = {
  tiles: Record<string, LearningTileDefinition>
  graphicTemplates: Record<LearningGraphicTemplateId, LearningGraphicTemplate>
  categoryStyles: Record<LearningTileCategory, LearningCategoryStyle>
  questionBank: Record<string, LearningModule>
  rewards: LearningRewardConfig
}
...
const DEFAULT_CONFIG: LearningConfig = {
  ...
  graphicTemplates: {
    aurora: {
      ...
      shimmerClass:
        'bg-[conic-gradient(from_120deg,_rgba(255,255,255,0.25),_rgba(255,255,255,0)_40%,_rgba(255,255,255,0.2)_60%,_rgba(255,255,255,0)_85%)]',
      sparkleClass:
    ...
    prism: {
      ...
      shimmerClass:
        'bg-[conic-gradient(from_40deg,_rgba(255,255,255,0.28),_rgba(255,255,255,0)_38%,_rgba(255,255,255,0.18)_62%,_rgba(255,255,255,0)_85%)]',
      sparkleClass:
    ...
    comet: {
      ...
      shimmerClass:
        'bg-[conic-gradient(from_210deg,_rgba(255,255,255,0.25),_rgba(255,255,255,0)_40%,_rgba(255,255,255,0.2)_65%,_rgba(255,255,255,0)_85%)]',
      sparkleClass:
...
const normalizeRecord = <T>(value: unknown, fallback: T): T => {
...
const normalizeRewards = (config: Partial<LearningRewardConfig> | undefined): LearningRewardConfig => {
  return {
    baseStars: coerceNumber(config?.baseStars, DEFAULT_CONFIG.rewards.baseStars),
    baseXp: coerceNumber(config?.baseXp, DEFAULT_CONFIG.rewards.baseXp),
    streakBonusStars: coerceNumber(config?.streakBonusStars, DEFAULT_CONFIG.rewards.streakBonusStars),
    streakBonusXp: coerceNumber(config?.streakBonusXp, DEFAULT_CONFIG.rewards.streakBonusXp),
    maxStreakBonusDays: coerceNumber(config?.maxStreakBonusDays, DEFAULT_CONFIG.rewards.maxStreakBonusDays),
  }
}

const parsedConfig = rawLearningConfig as Partial<LearningConfig>

const learningConfig: LearningConfig = {
  tiles: normalizeRecord(parsedConfig?.tiles, DEFAULT_CONFIG.tiles),
  graphicTemplates: normalizeRecord(parsedConfig?.graphicTemplates, DEFAULT_CONFIG.graphicTemplates),
  categoryStyles: normalizeRecord(parsedConfig?.categoryStyles, DEFAULT_CONFIG.categoryStyles),
  questionBank: normalizeRecord(parsedConfig?.questionBank, DEFAULT_CONFIG.questionBank),
  rewards: normalizeRewards(parsedConfig?.rewards),
}

export const LEARNING_TILE_DEFINITIONS = learningConfig.tiles
export const LEARNING_GRAPHIC_TEMPLATES = learningConfig.graphicTemplates
export const LEARNING_CATEGORY_STYLES = learningConfig.categoryStyles
export const LEARNING_QUESTION_BANK = learningConfig.questionBank
export const LEARNING_REWARD_CONFIG = learningConfig.rewards
