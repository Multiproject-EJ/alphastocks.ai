...
- [x] Slice 8 — Config-first jump-threshold tuning (promote threshold into shared config)
...
- 2026-02-14: Completed Slice 8 by moving `jumpThreshold` into shared event-track config (`EVENT_TRACK_PROGRESS_CONFIG`) and routing `getEventTrackDefinition` through that config with override support for tuning experiments. Added focused config tests for default and override behavior.
- Next step: Pick Slice 9 scope (if needed) for additional pacing knobs after validating jump-threshold tuning telemetry.
